...
%s`, green, reset)
	...
	// 打印使用示例（帮助用户快速上手）
	fmt.Println("\n【使用示例】")
	fmt.Println("  1. 基础扫描（默认并发10，深度3）")
	fmt.Println("     .\\gofus.exe -u https://example.com -w dict.txt")
	fmt.Println("  2. 过滤404 + 深度2 + 并发20")
	fmt.Println("     .\\gofus.exe -u example.com -w dict.txt -fc 404 -d 2 -c 20")
	fmt.Println("  3. 过滤403/404 + 过滤长度2048")
	fmt.Println("     .\\gofus.exe -u example.com -w dict.txt -fc 403,404 -fl 2048")
}
